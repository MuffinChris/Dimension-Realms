	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (sender instanceof Player) {
			...
			if (args.length == 0) {
			    ...
			    Main.msg(p, "&8» &c&lKDR: &f" + kdr + " &8(&c" + df.format(kills) + "&8/&c" + deaths + "&8)");
				...
				if (Bukkit.getPlayer(args[0]) instanceof Player) {
				    ...
				    double kdr = (1.0 * kills) / (1.0 * deaths);
				    DecimalFormat df = new DecimalFormat("#.##");
				    Main.msg(p, "&8» &c&l" + t.getName() + "'s KDR: &f" + kdr + " &8(&c" + df.format(kills) + "&8/&c" + deaths + "&8)");
				} else {
			...
			if (args.length == 1) {
				if (Bukkit.getPlayer(args[0]) instanceof Player) {
				    ...
				    double kdr = (1.0 * kills) / (1.0 * deaths);
				    DecimalFormat df = new DecimalFormat("#.##");
				    Main.so("&8» &c&l" + t.getName() + "'s KDR: &f" + kdr + " &8(&c" + df.format(kills) + "&8/&c" + deaths + "&8)");
				} else {
