...
public class PartyCommand implements CommandExecutor, Listener {
    ...
    public void sendPlayerInv(Player p) {
        ...
        lore.add(Main.color("&8» &eLVL: &f" + main.getLevel(p)));
        lore.add(Main.color("&8» &aXP: &f" + df.format((100.0 * main.getExp(p)) / (1.0 * main.getExpMax(p))) + "%"));spMeta.setLore(lore);
        ...
        for (int i = 2; i < party.getPlayers().size() + 2; i++) {
            ...
            if (t.equals(p)) {
                ...
                lore.add(Main.color("&8» &eLVL: &f" + main.getLevel(t)));
                lore.add(Main.color("&8» &aXP: &f" + df.format((100.0 * main.getExp(t)) / (1.0 * main.getExpMax(t))) + "%"));
                smeta.setLore(lore);
